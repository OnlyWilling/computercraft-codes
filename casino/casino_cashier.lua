...
local custom_computer_id = os.getComputerID()
casinoNetwerk.open(custom_computer_id)
...
function changeBalance(amount, userId)
    ...
    function sendCommandLoop()
        while true do
            casinoNetwerk.transmit(1, custom_computer_id,
                textutils.serialiseJSON({ ["command"] = "changeBal", ["userId"] = userId, ["attributes"] = amount }))
            sleep(0.5)
        end
    end
    ...
    parallel.waitForAny(sendCommandLoop, receiveConfirmation, timeoutCheck)

    -- local event, side, channel, replyChannel, message, distance = os.pullEvent("modem_message")
    if msg ~= nil then
        print("Success! Received: "..tostring(msg))
        disk.setLabel(peripheral.getName(drive), tostring(getBalance(userId)) .. " chips")
        return msg
    else
        print("Error: Transaction timed out.")
        return nil
    end
end
...
function getBalance(userId)
    function sendCommandLoop()
        while true do
            casinoNetwerk.transmit(1, custom_computer_id,
                textutils.serialiseJSON({ ["command"] = "getBal", ["userId"] = userId, ["attributes"] = nil }))
            sleep(0.5)
        end
    end
...
function getChipPrice()
    function sendCommandLoop()
        while true do
            casinoNetwerk.transmit(1, custom_computer_id,
                textutils.serialiseJSON({ ["command"] = "getChipPrice", ["userId"] = nil, ["attributes"] = nil }))
            sleep(0.5)
        end
    end
...
function getWithdrawalFee()
    function sendCommandLoop()
        while true do
            casinoNetwerk.transmit(1, custom_computer_id,
                textutils.serialiseJSON({ ["command"] = "getWithdrawalFee", ["userId"] = nil, ["attributes"] = nil }))
            sleep(0.5)
        end
    end
...
function getNewToken()
    function sendCommandLoop()
        while true do
            casinoNetwerk.transmit(1, custom_computer_id, textutils.serialiseJSON({ ["command"] = "getNewToken" }))
            sleep(0.5)
        end
    end
