...
-- ==== 新增的调试代码 开始 ====
print("File content length: " .. tostring(#content))
if not content or #content == 0 then
  error("Error: The file is empty inside the game!")
else
  print("File starts with: ")
  print(string.sub(content, 1, 100)) -- 打印文件的前100个字符
end
-- ==== 新增的调试代码 结束 ====
local img = textutils.unserialize(content)
if not img then
  error("Unserialize failed. File content is invalid.")
end
